...
function initTrees() {
  ...
  // Data for buttons: each with an ID (1-5) and corresponding color
  const buttonsData = [
    { id: 1, color: sentenceColors[0] },
    { id: 2, color: sentenceColors[1] },
    { id: 3, color: sentenceColors[2] },
    { id: 4, color: sentenceColors[3] },
    { id: 5, color: sentenceColors[4] }
  ];
  ...
  // Load the treebank JSON file
  fetch('./tree.json')
    .then(response => response.json())
    .then(data => {
      // Save JSON data globally for access in click handlers
      window.treeData = data;

      // Attach click handlers after data is loaded
      buttons.on("click", (event, d) => {
        renderTree(window.treeData[d.id], d.color);
      });

      // Render the first sentence by default
      renderTree(data["1"], buttonsData[0].color);
    })
    .catch(error => {
      console.error("Error loading tree JSON data:", error);
    });
}
